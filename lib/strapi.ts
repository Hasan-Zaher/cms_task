...
class StrapiAPI {
  ...
  async getTeamMembers(): Promise<TeamMember[]> {
    const response = await this.request<StrapiResponse<StrapiItem[]>>("/team-members?populate=*")
  ...
  async getServices(): Promise<Service[]> {
    const response = await this.request<StrapiResponse<StrapiItem[]>>("/services?populate=*")
    return response.data.map((item) => ({
      id: item.id,
  ...
  async getBlogPosts(): Promise<BlogPost[]> {
    const response = await this.request<StrapiResponse<StrapiItem[]>>("/blog-posts?populate=*&sort=createdAt:desc")
    return response.data.map((item) => ({
      id: item.id,
      title: item.attributes.title,
  ...
  async getTestimonials(): Promise<Testimonial[]> {
    const response = await this.request<StrapiResponse<StrapiItem[]>>("/testimonials?populate=*")
    return response.data.map((item) => ({
      id: item.id,
      name: item.attributes.name,
      position: item.attributes.position,
  ...
  async getHeroSlides(): Promise<HeroSlide[]> {
    const response = await this.request<StrapiResponse<StrapiItem[]>>("/hero-slides?populate=*&sort=order:asc")
    return response.data.map((item) => ({
      id: item.id,
      title: item.attributes.title,
      description: item.attributes.description,
      backgroundImage: item.attributes.backgroundImage?.data?.attributes?.url || null,
