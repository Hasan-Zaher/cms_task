...
export function TeamSection() {
  ...
  const nextSlide = () => {
    setCurrentIndex((prev) =>
      prev + itemsPerPage >= teamMembers.length ? 0 : prev + itemsPerPage
    );
  };
  ...
  return (
    <section className="py-16 bg-gray-50">
      <div className="container mx-auto px-4">
        ...
        >
          <h2 className="text-3xl text-brown md:text-4xl font-bold   mb-4 text-center">
            {t("ourTeam")}
          </h2>
          <p className="text-gray-600 max-w-2xl mx-auto text-pretty text-center">
            {t("teamDescription")}
          </p>
        </div>

        {/* Team Members */}
        <div className="relative px-5 lg:px-30">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {teamMembers.length > itemsPerPage && (
            <>
              <Button
                ...
                } top-1/2 transform -translate-y-1/2 bg-white shadow-lg  hover:shadow-xl  cursor-pointer w-10 h-10 p-0 rounded-full`}
              >
                {isRTL ? (
